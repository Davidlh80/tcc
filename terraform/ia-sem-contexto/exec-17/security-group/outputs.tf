output "security_group_id" {
  description = "ID do Security Group."
  value       = aws_security_group.this.id
}

output "security_group_arn" {
  description = "ARN do Security Group."
  value       = aws_security_group.this.arn
}

output "security_group_name" {
  description = "Nome do Security Group."
  value       = aws_security_group.this.name
}

output "security_group_vpc_id" {
  description = "ID da VPC associada ao Security Group."
  value       = aws_security_group.this.vpc_id
}

output "ingress_rules_count" {
  description = "Quantidade total de regras de ingress configuradas."
  value       = local.ingress_rules_count
}

output "egress_rules_count" {
  description = "Quantidade total de regras de egress configuradas."
  value       = local.egress_rules_count
}

output "security_group_tags" {
  description = "Tags aplicadas ao Security Group."
  value       = aws_security_group.this.tags_all
}
